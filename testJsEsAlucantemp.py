...
if os.path.getsize(objectRecieved['fileReceived']) != 0:
    if objectRecieved['db']['siteConfig']['siteInfo']['siteDeployed'] is True:
        if os.path.splitext(objectRecieved['fileReceived'])[-1] and len(objectRecieved['db']['siteConfig']['js']) != 0:
            if os.path.basename(objectRecieved['fileReceived']).startswith('INVERTER') or os.path.basename(
                    objectRecieved['fileReceived']).startswith('ERROR') or os.path.basename(
                    objectRecieved['fileReceived']).startswith('LOG') or os.path.basename(
                    objectRecieved['fileReceived']).startswith('METER') or os.path.basename(
                ...
                elif os.path.basename(objectRecieved['fileReceived']).startswith('ERROR'):
                    try:
                        dictionary.__delitem__('DAY_ENERGY')
                        dictionary.__delitem__('TOTAL_ENERGY')
                        dictionary.__delitem__('PAC')
                ...
                elif os.path.basename(objectRecieved['fileReceived']).startswith('LOG'):
                    try:
                        dictionary.__delitem__('DAY_ENERGY')
                        dictionary.__delitem__('TOTAL_ENERGY')
                        dictionary.__delitem__('PAC')
